...
const Navbar = () => {
  ...
  const [categories, setCategories] = useState<CategoriesByGender>({})
  ...
  useEffect(() => {
    const getCategories = async () => {
      const data = await CategoryService.getCategoriesByGender();
      if (data) {
        setCategories(data)
      }
    }
  ...
  return (
    <nav className={`grid grid-cols-3 border-b -border--color-very-light-grey h-20 shadow-lg -shadow--color-very-light-grey z-40 -bg--color-white transition-transform duration-500 fixed w-screen px-4 top-0 ${scrollDown && !isOpenCart && " scroll-down shadow-none"} ${scrollUp && " scroll-up"}`}>
      ...
      <ul className="invisible hidden w-full text-center h-full lg:visible lg:relative lg:block bg-white">
        {
          ...
          Object.keys(categories).map(gender => {
            return (
                <li className="inline-block hover:border-b-2 w-24 h-full hover:cursor-pointer font-semibold group">
                  <h1 className="h-full">
                    <p className="h-full flex items-center justify-center">{gender.toUpperCase()}</p>
                  </h1>
                  <section className="hidden group-hover:grid hover:grid place-content-start absolute -z-20 top-20 w-full left-0 bg-white border -border--color-very-light-grey open-submenu">
                    <div className="col-span-1 p-4 text-start">
                      <h1 className="font-bold text-lg -text--color-black">
                        Products
                      </h1>
                      <ul className="">
                        {
                          categories[gender].map((category: Category) => {
                            return (
                              <li className="font-medium py-1 -text--color-greyest-violet hover:-text--color-dark-grey-violet">
                                <Link to={`${gender.toLowerCase()}/${category.name.toLowerCase()}`}>{category.name}</Link>
                              </li>
                            )
                          })
                        }
